...
import os
import abs_competition_maker_lbda as scm
import abs_competition_maker_svm as lcm
import seo.letor_fold_creator_min_max_normalize as lfc
import seo.query_to_fold as qtf
from model_running import cross_validator as cv
import abs_stat_handler_svm as aashs
import abs_stat_handler_lbda as aashl
import seo.relevance_index as ri
...
def lbda_simulation(chosen_models, data_set_location, query_to_fold_index, score_file, c_d_loc, new_scores_path, models_path, rel_index,budget_creator):
    ...
    c = scm.competition_maker(15, budget_creator, score_file, 10, data_set_location, 0.1, chosen_models, query_to_fold_index, c_d_loc + "/" + budget_creator.model, new_scores_path + "/" + budget_creator.model, models_path)
    return c.competition("/lv_local/home/sgregory/LTOR_MART_min_max/new_scores_svml/"+budget_creator.model+"/final",rel_index)

def svm_simulation(chosen_models,data_set_location,query_to_fold_index,score_file,rel_index,budget_creator):
    c = lcm.competition_maker(15, budget_creator, score_file, 10, data_set_location, 0.1, chosen_models, query_to_fold_index)
    return c.competition(rel_index)
...
if __name__ == "__main__":
    ...
    lbda_score_file = svm_score_file
    rel_index = ri.relevance_index("qrels")
    rel_index.create_relevance_index()
    pool = p(3)

    gg = aashl.abs_stat_handler_lbda("01", 0.1,c)
    aa = aashl.abs_stat_handler_lbda("005", 0.05,c)
    bb = aashl.abs_stat_handler_lbda("001", 0.01, c)
    svm_gg = aashs.abs_stat_handler_svm("01",0.1)
    svm_aa = aashs.abs_stat_handler_svm("005", 0.05)
    svm_bb = aashs.abs_stat_handler_svm("001", 0.01)
    ...
    lbda_f = partial(lbda_simulation, lbda_chosen_models, data_set_location, q.query_to_fold_index, lbda_score_file, "/lv_local/home/sgregory/LTOR_MART_min_max/competition_svml", "/lv_local/home/sgregory/LTOR_MART_min_max/new_scores_svml/", "/lv_local/home/sgregory/LTOR_MART_min_max/models/LAMBDAMART/",rel_index.rel_index)
    ...
    write_res_to_file(lbda_final_results, "LAMDAMART_abs_svm")
    ...
    write_res_to_file(svm_final_results,"SVM_abs_svm")
